  ...
  <div class="pcs-template-body">
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="pcs-itemtable" style="width:100%;margin-top:20px;table-layout:fixed;">
      ...
      <tr style="height:32px;">
        <td class="pcs-itemtable-header pcs-itemtable-breakword" id="" style="padding: 5px 0px 5px 5px;width: 10%;text-align: center;">
          Fecha
        </td>
        <td class="pcs-itemtable-header pcs-itemtable-breakword" id="" style="padding: 5px 10px 5px 20px;width: 10%;text-align: left;">
          Nombre
        </td>
        <td class="pcs-itemtable-header pcs-itemtable-breakword" id="" style="padding: 5px 10px 5px 5px;width: 20%;text-align: right;">
          Contacto
        </td>
        ...
        @foreach($ordenes as $key => $item)
          <tr>
            ...
            <td class="pcs-item-row" style="padding: 10px 0px 10px 20px;" valign="top">
              {{ $item->nombre_visualizacion }}
            </td>
            <td class="pcs-item-row lineitem-column text-align-right">
              {{ isset($item->cp_nombres) ? $item->cp_nombres: '' }}
              {{ isset($item->cp_apellido_paterno) ? $item->cp_apellido_paterno: '' }}
              {{ isset($item->cp_apellido_materno) ? $item->cp_apellido_materno: '' }}
            </td>
            <td class="pcs-item-row lineitem-column text-align-right">
              {{ $item->cp_correo_electronico }}
            </td>
            <td class="pcs-item-row lineitem-column lineitem-content-right text-align-right">
              {{ $item->cp_telefono_trabajo }}
            </td>
          </tr>
        @endforeach
      @endif
