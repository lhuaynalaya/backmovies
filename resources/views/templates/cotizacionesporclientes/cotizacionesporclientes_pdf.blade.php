  ...
  <div class="pcs-template-body">
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="pcs-itemtable" style="width:100%;margin-top:20px;table-layout:fixed;">
      ...
      <tr style="height:32px;">
        <td class="pcs-itemtable-header pcs-itemtable-breakword" id="" style="padding: 5px 0px 5px 5px;width: 30%;text-align: center;">
          Cliente
        </td>
      ...
      @if(isset($ventas))
        @foreach($ventas as $key => $item)
          <tr>
            ...
            <td class="pcs-item-row lineitem-column text-align-right">
              {{ isset($item->cp_nombres) ? $item->cp_nombres: '' }} {{ isset($item->cp_apellido_paterno) ? $item->cp_apellido_paterno: '' }} {{ isset($item->cp_apellido_materno) ? $item->cp_apellido_materno: '' }}
            </td>
            <td class="pcs-item-row lineitem-column lineitem-content-right text-align-right">
              {{ isset($item->cp_correo_electronico) ? $item->cp_correo_electronico: '' }}
            </td>
            <td class="pcs-item-row lineitem-column lineitem-content-right text-align-right">
              {{ isset($item->cp_celular) ? $item->cp_celular: '' }} {{ isset($item->cp_telefono_trabajo) ? $item->cp_telefono_trabajo: '' }}
            </td>
            <td class="pcs-item-row lineitem-column lineitem-content-right text-align-right">
              {{ isset($item->total) ? $item->total: '' }}
            </td>
            <td class="pcs-item-row lineitem-column lineitem-content-right text-align-right">
              {{ isset($item->total_formato) ? $item->total_formato: '' }}
            </td>
          </tr>
        @endforeach
      @endif
